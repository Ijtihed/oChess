...
// Slots
const FIRE_AURA_SLOT = "const phase=Math.sin(t*0.006)*0.5+0.5; const r=24+phase*8; const g=ctx.createRadialGradient(0,0,0,0,0,r); g.addColorStop(0,'rgba(255,235,120,0.42)'); g.addColorStop(0.45,'rgba(255,90,0,0.22)'); g.addColorStop(1,'rgba(80,0,0,0)'); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(0,0,r,0,Math.PI*2); ctx.fill(); for(let i=0;i<5;i++){ const a=i*1.256+t*0.004; ctx.strokeStyle='rgba(255,140,0,0.5)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(Math.cos(a)*14,Math.sin(a)*14); ctx.lineTo(Math.cos(a)*28,Math.sin(a)*28); ctx.stroke(); }";
const FIRE_WEAPON_SLOT = "ctx.save(); ctx.rotate(0.35*facing); const flick=Math.sin(t*0.012)*3; ctx.strokeStyle='rgba(110,45,0,0.9)'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(9*facing,-16); ctx.lineTo(21*facing,-2); ctx.stroke(); const g=ctx.createRadialGradient(24*facing,-2,0,24*facing,-2,9+flick); g.addColorStop(0,'rgba(255,255,180,0.95)'); g.addColorStop(0.45,'rgba(255,120,0,0.78)'); g.addColorStop(1,'rgba(255,0,0,0)'); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(24*facing,-2,10+flick,0,Math.PI*2); ctx.fill(); ctx.restore();";
...
// Projectiles
const FIREBALL_PROJECTILE = "const dx=p.toX-p.fromX,dy=p.toY-p.fromY; const len=Math.sqrt(dx*dx+dy*dy)||1; for(let i=0;i<8;i++){ const back=i/8; const tx=p.x-(dx/len)*back*22; const ty=p.y-(dy/len)*back*22; ctx.fillStyle='rgba(255,'+(190-i*16)+',0,'+(0.85-back)+')'; ctx.beginPath(); ctx.arc(tx,ty,7-i*0.55,0,Math.PI*2); ctx.fill(); } const g=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,14); g.addColorStop(0,'rgba(255,255,170,0.95)'); g.addColorStop(0.45,'rgba(255,110,0,0.85)'); g.addColorStop(1,'rgba(255,0,0,0)'); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(p.x,p.y,14,0,Math.PI*2); ctx.fill();";
...
const FIRE_IMPACT_EFFECT = "const a=1-e.progress; ctx.strokeStyle='rgba(255,100,0,'+(0.75*a)+')'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(e.x,e.y,8+e.progress*35,0,Math.PI*2); ctx.stroke();";
